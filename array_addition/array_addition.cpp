...
void       seqSumArrays( long* a, long* b, long* c, const long N );  // Sequentially performs the N-dimensional operation c = a + b.
void       parSumArrays( long* a, long* b, long* c, const long N );  // Parallelly performs the N-dimensional operation c = a + b.
bool       checkEquality( long* c1, long* c2, const long N );        // Check if the N-dimensional arrays c1 and c2 are equal.
...
int main() {
    ...
    long                ARRAYS_DIM = 1 << 20;
    std::vector< long > a( ARRAYS_DIM, 3 );
    std::vector< long > b( ARRAYS_DIM, 5 );

    /**
     * Prepare sequential and parallel outputs.
     * */

    std::vector< long > cs( ARRAYS_DIM );
    std::vector< long > cp( ARRAYS_DIM );
...
void initializeDevice() {
    ...
    std::ifstream kernel_file( "../../array_addition/array_addition.cl" );
...
void seqSumArrays( long* a, long* b, long* c, const long N ) {
    for ( long i = 0; i < N; i++ ) {
        c[ i ] = a[ i ] + b[ i ];
    }
}
...
void parSumArrays( long* a, long* b, long* c, const long N ) {

    /**
     * Create buffers and allocate memory on the device.
     * */

    cl::Buffer aBuf( context, CL_MEM_READ_ONLY | CL_MEM_HOST_NO_ACCESS | CL_MEM_COPY_HOST_PTR, N * sizeof( long ), a );
    cl::Buffer bBuf( context, CL_MEM_READ_ONLY | CL_MEM_HOST_NO_ACCESS | CL_MEM_COPY_HOST_PTR, N * sizeof( long ), b );
    cl::Buffer cBuf( context, CL_MEM_WRITE_ONLY | CL_MEM_HOST_READ_ONLY, N * sizeof( long ) );
    ...
    queue.enqueueReadBuffer( cBuf, CL_TRUE, 0, N * sizeof( long ), c );
}
...
bool checkEquality( long* c1, long* c2, const long N ) {
    for ( long i = 0; i < N; i++ ) {
        if ( c1[ i ] != c2[ i ] ) {
            return false;
        }
